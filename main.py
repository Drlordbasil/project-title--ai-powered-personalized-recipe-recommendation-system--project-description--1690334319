...
# Constants
COLUMNS = ['cuisine_types', 'dietary_restrictions', 'cooking_skill', 'favorite_ingredients']
...
# Create user profile
def create_profile():
    cuisine_types = input('Enter your preferred cuisine types (separated by commas): ').split(',')
    dietary_restrictions = input('Enter your dietary restrictions (separated by commas): ').split(',')
    cooking_skill = input('Enter your cooking skill level (beginner, intermediate, advanced): ')
...
def filter_recipes(recipes_df, user_profile):
    ...
    filtered_recipes_df = filtered_recipes_df[filtered_recipes_df[COLUMNS[0]].isin(user_profile['cuisine_types'])]

    # Filter based on dietary restrictions
    filtered_recipes_df = filtered_recipes_df[~filtered_recipes_df[COLUMNS[1]].isin(user_profile['dietary_restrictions'])]

    # Filter based on cooking skill
    filtered_recipes_df = filtered_recipes_df[filtered_recipes_df[COLUMNS[2]] == user_profile['cooking_skill']]
...
def calculate_similarity(filtered_recipes_df):
    ...
    count_matrix = count_vectorizer.fit_transform(filtered_recipes_df['ingredients'].values.astype('U'))
...
def get_recommendations():
    recipe_indices = similarity_matrix[-1].argsort()[::-1][1:NUM_RECOMMENDATIONS + 1]
